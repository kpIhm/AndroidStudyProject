...
public class CustomToastActivity extends AppCompatActivity implements View.OnClickListener
{
	...
	public void onClick(View v)
	{
		// 빨강버튼을 클릭하면 빨강 토스트메시지가, 초록을 클릭하면 초록 토스트메시지가, 파랑을 클릭하면 파랑 토스트메시지가 나오도록 해주세요.
		// 되신분들은 오프채팅방에 올려주세요 ^^
		Typeface typeface = Typeface.createFromAsset(getAssets(),"fonts/yeongdo.ttf");


		if (v.getId()== R.id.btn_custom_toast_red){
		CustomToast.makeToast(activity, "basic first message").setTextColor(Color.RED)
		           .setTextTypeface(typeface)  .show();
		}

		else if (v.getId()== R.id.btn_custom_toast_green){
			CustomToast.makeToast(activity, "basic first message").setTextColor(Color.GREEN).show();}
		else if (v.getId()== R.id.btn_custom_toast_blue){
			CustomToast.makeToast(activity, "basic first message").setTextColor(Color.BLUE).show();}
	}
}